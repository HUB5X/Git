...
# IMPORTANT: Import get_flag_from_name from utils
from utils import card_pattern, get_bin_details, parse_expiry_year, get_flag_from_name 
...
def settings_view(message):
    ...
    bot.send_message(message.chat.id, "⚙️ **Settings Panel**", reply_markup=markup, parse_mode="Markdown")
...
def country_list_view(call):
    ...
    msg_text += "━━━━━━━━━━━━━━━━━━\n"
    msg_text += "မိမိလိုချင်သော နိုင်ငံ၏ **နံပါတ်စဉ်** ကို Reply ပြန်ပေးပါ။\n\n"
    
    user_id = call.from_user.id
    user_selection_cache[user_id] = {}
    
    for idx, (country, db_flag, count) in enumerate(data):
        # FIX: Database က Flag က အဖြူဖြစ်နေရင် (သို့) မရှိရင် utils ကနေ တွက်ယူမယ်
        display_flag = db_flag
        if not display_flag or display_flag == "🏳️":
            display_flag = get_flag_from_name(country)
    ...
    nav_btns = []
    if page > 1:
        nav_btns.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"country_page_{page-1}"))
    if page < total_pages:
        nav_btns.append(InlineKeyboardButton("Next ➡️", callback_data=f"country_page_{page+1}"))
...
def file_tools_view(message):
    ...
    bot.send_message(message.chat.id, "📂 **File Processing Tools**", reply_markup=markup, parse_mode="Markdown")

@bot.callback_query_handler(func=lambda call: call.data == "upload_req")
def upload_prompt(call):
    try:
        msg = bot.edit_message_text(
            "📤 **Please send the .txt file now:**\n(Reply with a combo file)", 
            call.message.chat.id, 
            call.message.message_id
        )
    except:
        msg = bot.send_message(call.message.chat.id, "📤 **Please send the .txt file now:**")
    bot.register_next_step_handler(msg, process_upload_step)

def process_upload_step(message):
    if not message.document:
        bot.reply_to(message, "❌ Invalid file. Please try again.")
        return
    
    status_msg = bot.send_message(message.chat.id, "⏳ **Initializing Upload...**")
...
async def process_bulk_upload_live(chat_id, message_id, file_path):
    ...
    total = len(matches)
    if total == 0:
        bot.edit_message_text("❌ **Error:** No valid cards found.", chat_id, message_id)
        return
...
def list_remove_channels(call):
    ...
    for cid in MONITORED_CHANNELS:
        markup.add(InlineKeyboardButton(f"🗑 Remove ID: {cid}", callback_data=f"rem_{cid}"))
...
def add_from_list_init(call):
    bot.edit_message_text("⏳ **Fetching your dialogs...**", call.message.chat.id, call.message.message_id)
    ...
    try:
        ...
        for d in dialogs:
            if d.is_group or d.is_channel:
                ...
                count += 1
        bot.send_message(chat_id, f"📂 **Select to Monitor:**", reply_markup=markup)
    except Exception as e:
...
def save_channel_id(call):
    ...
    bot.send_message(call.message.chat.id, f"✅ Added ID `{new_id}`")
...
def process_bin_lookup(message):
    ...
    if stats and stats['count'] > 0:
        ...
        msg = (
            f"🔍 **BIN Information Found**\n\n"
            f"🔢 BIN: `{bin_code}`\n"
            f"📊 Cards: `{stats['count']}`\n"
            f"{flag} Country: {stats['country']}\n"
            f"🏦 Bank: {stats['bank']}\n"
            f"💠 Type: {stats['type']} - {stats['level']}"
        )
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton(f"📥 Export {stats['count']} Cards", callback_data=f"do_export_bin_{bin_code}"))
        bot.send_message(message.chat.id, msg, reply_markup=markup, parse_mode="Markdown")
    else:
        bot.send_message(message.chat.id, f"❌ No cards found for BIN `{bin_code}`.")
