...
# --- CONFIGURATION ---
# File Upload တင်တဲ့အခါ တပြိုင်နက် ကဒ်ဘယ်နှစ်ခု စစ်မလဲ (5 is safe for API)
...
def show_dialog_page(chat_id, page):
    ...
    for item in current_items:
        ...
        status = "✅" if item['id'] in MONITORED_CHANNELS else ""
...
def dialog_page_switch(call):
    page = int(call.data.split("_")[2])
    # Call internal render function logic directly using edit
    items = dialog_cache.get(call.message.chat.id, [])
    if not items:
        bot.answer_callback_query(call.id, "List expired. Please fetch again.")
        return
    ...
    for item in current_items:
        status = "✅" if item['id'] in MONITORED_CHANNELS else ""
        markup.add(InlineKeyboardButton(f"{status} {item['title'][:20]}", callback_data=f"save_id_{item['id']}_{page}"))
        
    nav_btns = []
    if page > 1: nav_btns.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"dlg_page_{page-1}"))
    if page < total_pages: nav_btns.append(InlineKeyboardButton("Next ➡️", callback_data=f"dlg_page_{page+1}"))
    if nav_btns: markup.add(*nav_btns)
    markup.add(InlineKeyboardButton("🔙 Back", callback_data="add_channel_menu"))
    
    bot.edit_message_text(f"📂 **Select Channel (Page {page}/{total_pages}):**\nFound Total: {len(items)}", call.message.chat.id, call.message.message_id, reply_markup=markup)
...
def save_channel_id(call):
    ...
    # Re-trigger page render logic
    items = dialog_cache.get(call.message.chat.id, [])
    if items:
        # Re-build markup for current page
        per_page = 20
        total_pages = math.ceil(len(items) / per_page)
        ...
        for item in current_items:
            status = "✅" if item['id'] in MONITORED_CHANNELS else ""
            markup.add(InlineKeyboardButton(f"{status} {item['title'][:20]}", callback_data=f"save_id_{item['id']}_{current_page}"))
            
        nav_btns = []
        if current_page > 1: nav_btns.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"dlg_page_{current_page-1}"))
        if current_page < total_pages: nav_btns.append(InlineKeyboardButton("Next ➡️", callback_data=f"dlg_page_{current_page+1}"))
        if nav_btns: markup.add(*nav_btns)
        markup.add(InlineKeyboardButton("🔙 Back", callback_data="add_channel_menu"))
        
        bot.edit_message_text(f"📂 **Select Channel (Page {current_page}/{total_pages}):**\nFound Total: {len(items)}", call.message.chat.id, call.message.message_id, reply_markup=markup)
