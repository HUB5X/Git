...
def dashboard_view(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, f"⚠️ Error fetching stats: {e}")
...
async def resolve_and_add_channel(chat_id, link):
    try:
        ...
        if main_client:
            ...
            bot.send_message(chat_id, "❌ Main Client not ready.")
    except Exception as e:
...
def add_from_list_init(call):
    ...
    asyncio.run_coroutine_threadsafe(fetch_all_dialogs(call.message.chat.id, call.message.message_id), main_loop)

async def fetch_all_dialogs(chat_id, message_id):
    try:
        if not main_client:
            bot.send_message(chat_id, "❌ Client not connected.")
            return

        # Fetch unlimited dialogs
        ...
        # Save to Cache (items + message_id for safe editing)
        dialog_cache[chat_id] = {"items": channel_list, "message_id": message_id}
        
        # Show Page 1
        show_dialog_page(chat_id, 1, message_id)
        
    except Exception as e:
        bot.send_message(chat_id, f"❌ Error: {e}")

def show_dialog_page(chat_id, page, message_id=None):
    if chat_id not in dialog_cache or not isinstance(dialog_cache[chat_id], dict):
        bot.send_message(chat_id, "❌ Session expired. Please fetch again.")
        return

    state = dialog_cache[chat_id]
    ...
    for item in current_items:
        ...
        title = item['title'] if item['title'] else "Unknown"
        btn_text = f"{status} {title[:20]}" # Truncate title
    ...
    if message_id:
        try:
            ...
            dialog_cache[chat_id]["message_id"] = message_id
            return
        except: pass

    sent = bot.send_message(chat_id, msg_text, reply_markup=markup)
    try:
        dialog_cache[chat_id]["message_id"] = sent.message_id
    except: pass
...
def dialog_page_switch(call):
    page = int(call.data.split("_")[2])
    show_dialog_page(call.message.chat.id, page, call.message.message_id)
...
def save_channel_id(call):
    ...
    state = dialog_cache.get(chat_id)
    ...
    for item in items:
        if isinstance(item, dict) and item.get("id") == new_id:
            channel_name = item.get("title", "Unknown Channel")
            break
    ...
    # Refresh page
    try:
        show_dialog_page(chat_id, current_page, call.message.message_id)
    except TypeError:
